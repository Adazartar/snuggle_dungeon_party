...
PS
{
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float l_7 = dot( i.vNormalWs, l_6 );
		float l_8 = 1 - l_7;
		float3 l_9 = pow( 1.0 - dot( normalize( i.vNormalWs ), normalize( float3( l_8, l_8, l_8 ) ) ), l_5 );
		float3 l_10 = smoothstep( l_3, l_4, l_9 );
		float4 l_11 = g_vShadowColour;
		float4 l_12 = float4( l_10, 0 ) * l_11;
		float4 l_13 = l_2 * l_12;
		float l_14 = g_flF_HighlightSS_Min;
		float l_15 = g_flF_HighlightSS_Max;
		float l_16 = g_flF_HighlightPwr;
		float3 l_17 = pow( 1.0 - dot( normalize( i.vNormalWs ), normalize( CalculatePositionToCameraDirWs( i.vPositionWithOffsetWs.xyz + g_vHighPrecisionLightingOffsetWs.xyz ) ) ), l_16 );
		float4 l_18 = g_vFresnelColour;
		float4 l_19 = float4( l_17, 0 ) * l_18;
		float4 l_20 = smoothstep( l_14, l_15, l_19 );
